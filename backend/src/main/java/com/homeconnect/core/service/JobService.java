...
public class JobService {
        ...
        public EstimatePriceResponse estimatePrice(EstimatePriceRequest request) {
                log.info("Estimating price for service {} with {} hours", request.getServiceId(),
        ...
        public JobPostResponse createJobPost(CreateJobPostRequest request, Long customerId) {
                ...
                if (requestedWorkDateTime.isBefore(minAllowedDateTime)) {
                        throw new IllegalArgumentException("Thời gian làm việc phải đặt trước tối thiểu "
                                        + MIN_LEAD_TIME_HOURS + " tiếng");
                }

                GeocodingService.GeoResult geoResult = geocodingService
                                .geocodeByAddressText(request.getAddressDetail());
                String normalizedAddress = geoResult.getNormalizedAddress() != null
                                && !geoResult.getNormalizedAddress().isBlank()
                ...
                JobPost jobPost = JobPost.builder()
                                ...
                                .title(request.getTitle() != null ? request.getTitle()
                                                : "Cần " + service.getName() + " - " + request.getDurationHours()
                                                                + " giờ")
